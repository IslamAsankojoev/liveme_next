export const text = {
  header: {
    menu: [
      {
        text: {
          ru: 'Главная',
          en: 'Home',
          tr: 'Ana Sayfa',
          kg: 'Башкы бет',
          pl: 'Strona główna',
        },
        link: '/',
      },
      {
        text: {
          ru: 'Каталог',
          en: 'Shop',
          tr: 'Ürün',
          kg: 'Каталог',
          pl: 'Katalog',
        },
        link: '/shop',
      },
      {
        text: {
          ru: 'О нас',
          en: 'About us',
          tr: 'Hakkımızda',
          kg: 'Биз жөнүндө',
          pl: 'O nas',
        },
        link: '/about',
      },
      {
        text: {
          ru: 'Контакты',
          en: 'Contact',
          tr: 'İletişim',
          kg: 'Байланыш',
          pl: 'Kontakt',
        },
        link: '/contact',
      },
      {
        text: {
          ru: 'Вход / Регистрация',
          en: 'Login / Register',
          tr: 'Giriş / Kayıt ol',
          kg: 'Кирүү / Тиркелүү',
          pl: 'Zaloguj się / Zarejestruj',
        },
        link: '/register',
      },
      {
        text: {
          ru: 'Профиль',
          en: 'Profile',
          tr: 'Profil',
          kg: 'Профиль',
        },
        link: '/profile',
      },
    ],
  },
  buttonAddToCart: {
    ru: 'Добавить в корзину',
    en: 'Add to cart',
    tr: 'Sepete ekle',
    kg: 'Себетке кошуу',
    pl: 'Dodaj do koszyka',
  },
  footer: {
    about: {
      title: {
        ru: 'О нас',
        en: 'About us',
        tr: 'Hakkımızda',
        kg: 'Биз жөнүндө',
        pl: 'O nas',
      },
      subtitle: {
        ru: 'Мы предлагаем вам лучшие товары по самым низким ценам',
        en: 'We offer you the best products at the lowest prices',
        tr: 'En iyi ürünleri en uygun fiyatlara sunuyoruz',
        kg: 'Сизге эң мыкты продукттарды эң арзан баа менен сунуштайбыз',
        pl: 'Oferujemy najlepsze produkty w najbardziej przystępnych cenach',
      },
    },
    mailing: {
      title: {
        ru: 'Новости',
        en: 'News',
        tr: 'Haberler',
        kg: 'Жаңылыктар',
        pl: 'Aktualności',
      },
      subtitle: {
        ru: 'Узнавайте первыми о наших новстях',
        en: 'Be the first to know about our news',
        tr: 'Haberlerimiz hakkında ilk siz bilgi edinin',
        kg: 'Биздин жаңылыктар боюнча биринчилерден билиңиз',
        pl: 'Bądź pierwszym, który dowie się o naszych nowościach',
      },
      placeholder: {
        ru: 'Ваш email',
        en: 'Your email',
        tr: 'E-posta adresiniz',
        kg: 'Электрондук почтаңыз',
        pl: 'Adres e-mail',
      },
    },
    social: {
      title: {
        ru: 'Присоединяйтесь',
        en: 'Join us',
        tr: 'Bize katılın',
        kg: 'Бизге катталыңыз',
        pl: 'Dołącz do nas',
      },
      subtitle: {
        ru: 'Мы в социальных сетях',
        en: 'We are in social media',
        tr: 'Sosyal Medya Hesaplarımız',
        kg: 'Биз социалдык тармактарда',
        pl: 'Nasze konta w mediach społecznościowych',
      },
    },
    copyright: {
      text: {
        ru: 'Все права защищены',
        en: 'All rights reserved',
        tr: 'Tüm hakları saklıdır',
        kg: 'Бардык эрежелер сакталган',
        pl: 'Wszelkie prawa zastrzeżone',
      },
    },
  },
  empty: {
    title: {
      ru: 'Тут пусто',
      en: 'Nothing here',
      tr: 'Burada hiçbir şey yok',
      kg: 'Бул жерде эч нерсе жок',
      pl: 'Nic tu nie ma',
    },
    subtitle: {
      ru: 'Вы ещё не добавляли товар,перейдите на главную страницу.',
      en: 'Most likely, you have not added any products yet. To add a product, go to the main page.',
      tr: 'Ürün eklememişsiniz. Bir ürün eklemek için ana sayfaya gidin.',
      kg: 'Эч кандай продукт кошо элексиз, башкы беттен тандаңыз.',
      pl: 'Prawdopodobnie jeszcze nie dodałeś żadnych produktów. Aby dodać produkt, przejdź do strony głównej.',
    },
    button: {
      ru: 'Вернуться в каталог',
      en: 'Go to shop',
      tr: 'Kataloga git',
      kg: 'Каталогко кайтуу',
      pl: 'Przejdź do sklepu',
    },
  },
  notifications: {
    successOrder: {
      ru: 'Ваш заказ успешно оформлен',
      en: 'Your order has been successfully placed',
      tr: 'Siparişiniz başarıyla yerleştirildi',
      kg: 'Сиздин буйрутмаңыз кабыл алынды',
      pl: 'Twoje zamówienie zostało pomyślnie złożone',
    },
  },
};
